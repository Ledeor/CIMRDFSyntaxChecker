...
def cimAttributeContext(openTAG, iLine):
    ...
    if openTAG:
        ...
        if itrCIMAttrib > 1:
            ...
            print (">>> Error: Nested CIM attribute context in line " + str(iLine))
            return -1
    else:
        itrCIMAttrib = itrCIMAttrib - 1
...
def cimContext(openTAG, iLine):
    ...
    if openTAG:
        ...
        if itrCIMAttrib > 0:
            ...
            itrCIMClass = itrCIMClass - 1
            if itrCIMClass < 0:
                print (">>> Warning: Close CIM tag without open tag in line " + str(iLine))
                itrCIMClass = 0
...
def rdfContext(openTAG, iLine):
    ...
    if openTAG:
        ...
        itrRDF = itrRDF + 1
        if itrRDF > 1:
            print (">>> Warning: Nested RDF context in line " + str(iLine))
    else:
        itrRDF = itrRDF - 1
        if itrRDF < 0:
            print (">>> Warning: Close RDF tag without open tag in line " + str(iLine))
            itrRDF = 0
